...
const ModalAuthentication: ModalComponent<ModalAuthenticationProps> = (props) => {
  ...
  return (
    <div className={styles.wrapper}>
      <OutsideElementEvent onOutsideEvent={() => props.onClose()} style={{ width: '100%', maxWidth: 488, margin: `0 auto 0 auto` }}>
        <div className={styles.childModal} style={{ width: '100%', padding: 24 }}>
          <FormSubHeading>Sign in or create an account</FormSubHeading>
          ...
          <div style={{ marginTop: 24 }}>
            <ActionItem icon={`⭢`} active={props.active === 'PERSONAL'} onClick={() => modals.open(ModalForgotPassword)}>
              Forgot password?
            </ActionItem>
          </div>
          ...
          {loading ? null : (
            <Button loading={loading} href={`https://api.internet.dev/authenticate-google?domain=${Constants.OAUTH_REDIRECT_SIGNATURE}`} style={{ marginTop: 16, width: '100%' }}>
              <Google height="16px" style={{ marginRight: 12 }} /> Sign in with Google
            </Button>
          )}
          {loading ? null : (
            <Button loading={loading} href={`https://api.internet.dev/authenticate-apple?domain=${Constants.OAUTH_REDIRECT_SIGNATURE}`} style={{ marginTop: 16, width: '100%' }}>
          ...
          {showHandleInput && (
            <div style={{ marginTop: 24 }}>
              ...
              <Button
                href={`https://api.internet.dev/authenticate-bluesky?domain=${Constants.OAUTH_REDIRECT_SIGNATURE}&handle=${encodeURIComponent(blueskyHandle)}`}
